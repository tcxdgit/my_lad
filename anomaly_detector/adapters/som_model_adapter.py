...
# ANOMALY_COUNT = Gauge("aiops_lad_anomaly_count",
#                       "count of anomalies runs",
#                       ['anomaly_status'])
# ANOMALY_SCORE = Gauge("aiops_lad_anomaly_avg_score",
#                       "avg anomaly score")
...
#                                ['id'])
# ANOMALY_HIST = Histogram("aiops_hist",
#                          "histogram of anomalies runs")
# THRESHOLD = Gauge("aiops_lad_threshold",
#                   "Threshold of marker for anomaly")


class SomModelAdapter(BaseModelAdapter):
    """Self organizing map custom logic to train model.
    Includes logic to train and predict anomalies in logs."""

    def __init__(self, storage_adapter):
        """Init storage provider which provides config
         and storage interface with storage systems."""
        self.storage_adapter = storage_adapter
        update_model = self.storage_adapter.TRAIN_UPDATE_MODEL
        self.update_model = os.path.isfile(
            self.storage_adapter.MODEL_PATH) and update_model
        self.update_w2v_model = os.path.isfile(
            self.storage_adapter.W2V_MODEL_PATH) and update_model
    ...
    def train(self, node_map, data, recreate_model=True):
        ...
        # If node_map is none then we assume
        # it is calculating score for inference
        if recreate_model is True:
            self.model.set(
                np.random.rand(node_map,
                               node_map,
                               vectors.shape[1]))
        self.model.train(vectors,
                         node_map,
                         self.storage_adapter.TRAIN_ITERATIONS,
                         self.storage_adapter.PARALLELISM)
        dist = self.model.get_anomaly_score(vectors,
                                            self.storage_adapter.PARALLELISM)
    ...
    def predict(self, data, json_logs, threshold):
        """Prediction from data provided and
         if it hits threshold it flags it an anomaly."""
        ...
        logging.info("False Positive: %s ", false_positives)
        ...
        for i in range(len(data)):
            result = json_logs[i]
            result["predict_id"] = str(uuid.uuid4())
            result["anomaly_score"] = dist[i]
            # s["elast_alert"] = self.storage_adapter.ES_ELAST_ALERT
            result["inference_batch_id"] = inference_batch_id
            result["predictor_namespace"] = self.storage_adapter.OS_NAMESPACE
            result["e_message"] = quote(result["message"])
            # Record anomaly event in fact_store
            if dist[i] > threshold:
                if false_positives is not None:
                    if result["message"] in feedback_strategy.uniq_items:
                        logging.info('False positive was found'
                                     ' (score: %f): %s',
                                     dist[i], result["message"])
                        # FALSE_POSITIVE_COUNT.labels(id=s["predict_id"]).inc()
                        continue
                hist_count += 1
                result["anomaly"] = 1
                logging.warning("Anomaly found (score: %f): %s",
                                dist[i], result["message"])
                last_id[quote(result["message"])] = result["predict_id"]
    ...
    def process_anomaly_score(self, data):
        ...
        vector = self.w2v_model.one_vector(data)
        # dist = []
        dist = self.model.get_anomaly_score(vector,
                                            self.storage_adapter.PARALLELISM)
        dist = dist / max_dist
        return dist
    ...
    def set_threshold(self):
        ...
        logging.info("Models loaded, running %d infer loops",
                     self.storage_adapter.INFER_LOOPS)
